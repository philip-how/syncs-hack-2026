...
Sources:
  auto    - try Telstra first, then fall back to the government LIST mirror
...
def scrape_list(session: requests.Session, page_size: int = 2000, state: str | None = None) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    offset = 0
    while True:
        where = "1=1"
        if state:
            state_sql = state.upper().strip().replace("'", "''")
            where = f"STATE='{state_sql}'"
        params = {
            "where": where,
...
def main() -> int:
    ...
    ap.add_argument("--source", choices=["auto", "telstra", "list"], default="auto",
                    help="Data source. auto tries Telstra then falls back to the government LIST mirror.")
    ...
    with requests.Session() as session:
        if args.source == "list":
            rows = scrape_list(session, state=args.state)
        elif args.source == "telstra":
            rows = scrape_telstra(session)
        else:
            try:
                rows = scrape_telstra(session)
            except (requests.RequestException, RuntimeError) as exc:
                print(f"Telstra endpoints unavailable ({exc}); using government LIST mirror...", file=sys.stderr)
                rows = scrape_list(session, state=args.state)

    # Telstra's endpoint has no server-side state filter in this script, so filter
    # locally. LIST is already filtered server-side when --state is supplied.
    if args.state:
